...
export default function BottomNav() {
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <Link
        key={item.href}
        href={item.href}
        className="flex flex-col items-center gap-0.5 min-w-[48px] min-h-[36px] py-0.5 transition-colors active:scale-95"
      >
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around px-1 py-2 border-t"
      style={{
    ...
    >
      ...
      {/* Center Sell button */}
      <Link href="/create" className="flex flex-col items-center gap-0.5 -mt-3 min-h-[36px]">
        <div
          className="w-11 h-11 rounded-full flex items-center justify-center shadow-md border-2 transition-transform active:scale-90"
          style={{
            backgroundColor: 'var(--color-gold)',
            borderColor: 'var(--color-secondary-bg)',
          }}
        >
          <span style={{ color: '#0A0A0F', fontWeight: 900, fontSize: '22px', lineHeight: 1 }}>+</span>
        </div>
        <span
